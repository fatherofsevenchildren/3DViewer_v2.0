...
void GLWidget::SetData(std::shared_ptr<s21::ObjectData> input_data) {
  data_ = input_data;
}
...
void GLWidget::draw_points() {
  ...
  glVertexPointer(3, GL_DOUBLE, 0, data_.get()->vertexes.begin().base());
  glEnableClientState(GL_VERTEX_ARRAY);
  if (point_type)
    glEnable(GL_POINT_SMOOTH);
  else
    glDisable(GL_POINT_SMOOTH);
  glDrawArrays(GL_POINTS, 0, data_.get()->vertexes.size() / 3);
...
void GLWidget::draw_lines() {
  ...
  glVertexPointer(3, GL_DOUBLE, 0, data_.get()->vecs.begin().base());
  glEnableClientState(GL_VERTEX_ARRAY);
  if (dashed) {
  ...
  glDrawArrays(GL_LINES, 0, data_.get()->vectors.size());
  glDisableClientState(GL_VERTEX_ARRAY);
}
